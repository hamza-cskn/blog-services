...
export class BlogService {
  ...
  create(createBlogDto: CreateBlogDto) {
    ...
    blog.visible = true;
    return this.blogModel.create(blog).then(blog => BlogService.queryToBlog(blog));
  }

  findAll(options: FindAllOptions) {
    return this.blogModel
        //if showUnverified is true, show all blogs, otherwise only show verified blogs
        .find({visible: options.showUnverified ? {$in: [true, false, undefined]} : true})
  ...
  findOne(id: string) {
    return this.blogModel.findById(id).then(blog => BlogService.queryToBlog(blog))
  }

  update(id: string, updateBlogDto: UpdateBlogDto) {
    return this.blogModel.updateOne({_id: id}, updateBlogDto).then(blog => BlogService.queryToBlog(blog));
  }

  delete(id: string) {
    return this.blogModel.deleteOne({_id: id}).then(blog => BlogService.queryToBlog(blog));
  }

  private static queryToBlog(query: any): Blog {
    return {
      id: query._id,
